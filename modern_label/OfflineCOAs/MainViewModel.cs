...
namespace modern_coas
{
    class MainViewModel : INotifyPropertyChanged, INotifyDataErrorInfo
    {
        ...
        private ICommand oEM_offline_command;
        public ICommand OEM_offline_command
        {
            get
            {
                return oEM_offline_command ?? (oEM_offline_command = new CommandHandler(() => oem_offline_command_action(), _canExecute));
            }
        }
        ...
        public async void beta_command_action()
        {
            ...
            await _dialogCoordinator.ShowMessageAsync(this, "Message", "All Tasks Completed, Program will now Shutdown and Begins Backup");
            Application.Current.MainWindow.Close();
        }

     

        //put the current coas back into database and remove the entry from coas_history table
        ...
        //all the property changed event
            ...
            if (PropertyChanged != null)
            {
                ...
                switch (propName)
                {
                    ...
                    case ("Selected_channel"):
                        ...
                        Next_ocoa = mysql_data.get_next_ocoa(Selected_channel, Selected_station);
